...
import {
  LEGACY_GAMES_CUTOFF,
  LEGACY_GAMES_IGNORED_FOR_RATINGS,
...
export class RatingService {
  ...
  public async getPlayersRating(
    name: string,
  ): Promise<Rating | DefaultExceptionDto> {
    try {
  ...
  public parseFatigue(player: Player): Player {
    ...
    if (typeof player.fatigue === "string") {
      ...
      fatigueStr = fatigueStr.startsWith('"')
        ? fatigueStr.slice(1)
        : fatigueStr;
      fatigueStr = fatigueStr.endsWith('"')
        ? fatigueStr.slice(0, -1)
        : fatigueStr;
      ...
      try {
        ...
        this.logger.error(
          `Failed to parse fatigue for player ${player.id}:`,
          error,
        );
        player.fatigue = null;
      }
  ...
  public async generateRating(): Promise<void> {
    ...
    const playerRunner =
      this.playersRepository.manager.connection.createQueryRunner();
    const gameRunner =
    ...
    try {
      ...
      const playersData: Array<Player> =
        await this.playersRepository.query(getPlayersQuery);
      ...
      for (let i = 0; i < playersArray.length; i++) {
        ...
        if (
          playersArray[i].ratedgames > 0 ||
          (playersArray[i].unrated && playersArray[i].isbot)
        ) {
      ...
      this.logger.error(
        "Error processing player and game ratings, rolling back transaction. ",
        error,
      );
      ...
      if (!has_error) {
        ...
        writeFileSync(
          process.env.RATING_OUTPUT_PATH + "ratinglist.json.gz",
          gzipList,
        );
      }
    }
  ...
  public adjustedRating(
    ...
    if (player.rating < PARTICIPATION_CUTOFF + MAX_DROP) {
      return Math.min(
        player.rating,
        PARTICIPATION_CUTOFF + (MAX_DROP * participation) / PARTICIPATION_LIMIT,
      );
    } else {
      return Math.min(
        player.rating,
        player.rating - MAX_DROP * (1 - participation / PARTICIPATION_LIMIT),
      );
    }
  }
